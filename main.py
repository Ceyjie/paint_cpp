...
With Fill tool, in-app folder browser, and responsive shortcuts
"""
...
class PiPaint:
    def __init__(self):
        ...
        # Get current username for media path
        try:
        ...
        print("Pi Paint Started – Fill tool, responsive shortcuts")
    ...
    def create_toolbar(self):
        ...
        # Text buttons (bottom row) – now includes Fill
        btn_w, btn_h = 70, 40
        margin = 8
        y = 70
        x = 30
    ...
    def refresh_subdirs(self):
        try:
            ...
            self.subdirs = []
            for item in items:
                full = os.path.join(self.current_browse_path, item)
                if os.path.isdir(full):
                    self.subdirs.append(item)
            self.subdirs.sort(key=str.lower)
        except Exception as e:
            self.subdirs = []
            self.show_message(f"Can't read folder: {str(e)}")
    ...
    def go_media(self):
        if os.path.exists(self.media_path):
            self.current_browse_path = self.media_path
        elif os.path.exists("/media"):
            self.current_browse_path = "/media"
        else:
            self.show_message("Media folder not found")
            return
        self.refresh_subdirs()
        self.browse_scroll = 0
        self.selected_subdir = -1
    ...
    def draw_overlay(self):
        ...
        if self.browsing_folder:
            btn_w_small = 90
            spacing = 10
            total_width = 4 * btn_w_small + 3 * spacing
            start_x = win_x + (win_w - total_width) // 2

            home_btn = pygame.Rect(start_x, btn_y, btn_w_small, 40)
            pygame.draw.rect(self.screen, LIGHT_GRAY, home_btn)
            pygame.draw.rect(self.screen, BORDER_COLOR, home_btn, 2)
            self.screen.blit(self.font_small.render("Home", True, DARK_GRAY),
                             (home_btn.x + 15, home_btn.y + 10))
            self.home_btn_rect = home_btn

            media_btn = pygame.Rect(start_x + btn_w_small + spacing, btn_y, btn_w_small, 40)
            pygame.draw.rect(self.screen, LIGHT_GRAY, media_btn)
            pygame.draw.rect(self.screen, BORDER_COLOR, media_btn, 2)
            self.screen.blit(self.font_small.render("Media", True, DARK_GRAY),
                             (media_btn.x + 15, media_btn.y + 10))
            self.media_btn_rect = media_btn

            up_btn = pygame.Rect(start_x + 2*(btn_w_small + spacing), btn_y, btn_w_small, 40)
            pygame.draw.rect(self.screen, LIGHT_GRAY, up_btn)
            pygame.draw.rect(self.screen, BORDER_COLOR, up_btn, 2)
            self.screen.blit(self.font_small.render("Up", True, DARK_GRAY),
                             (up_btn.x + 25, up_btn.y + 10))
            self.up_btn_rect = up_btn

            select_btn = pygame.Rect(start_x + 3*(btn_w_small + spacing), btn_y, btn_w_small, 40)
            pygame.draw.rect(self.screen, (100, 200, 100), select_btn)
            pygame.draw.rect(self.screen, BORDER_COLOR, select_btn, 2)
            self.screen.blit(self.font_small.render("Select", True, WHITE),
    ...
    def handle_overlay_click(self, pos):
        ...
        # Keyboard toggle (save mode only)
        ...
        if self.browsing_folder:
            if hasattr(self, 'home_btn_rect') and self.home_btn_rect.collidepoint(pos):
                self.go_home()
                return True
            if hasattr(self, 'media_btn_rect') and self.media_btn_rect.collidepoint(pos):
                self.go_media()
                return True
    ...
    def handle_toolbar(self, btn):
        ...
        if typ == 'color':
            ...
            mode = "ON" if self.canvas.fill_mode else "OFF"
            self.show_message(f"Fill mode: {mode}")
        elif typ == 'background':
    ...
    def run(self):
        ...
        while self.running:
            ...
            # Color squares and text buttons
            color_idx = 0
            for btn in self.toolbar_buttons:
                if btn['type'] == 'color':
                    ...
                    if btn['type'] == 'fill' and self.canvas.fill_mode:
                        btn_color = (200, 255, 200)  # light green for active fill
            ...
            if self.canvas.fill_mode:
                tool = "Fill mode • Click to fill"
                preview = self.canvas.current_color
            elif self.canvas.eraser_mode:
